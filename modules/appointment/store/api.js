...
function getAppointmentByDate(payload) {
  return googleAuthAPI.get(`/primary/events?showDeleted=false&orderBy=startTime&singleEvents=true&maxResults=${payload.maxResults}&timeMin=${payload.datetime}`, {
    headers: {
...
function getAllAppointments(accessToken) {
  return googleAuthAPI.get("/primary/events?showDeleted=false&orderBy=startTime&singleEvents=true", {
    headers: {
      Authorization: `Bearer ${accessToken}`
    }
  });
