...
import { Modal, View, Text, ScrollView, TouchableOpacity, Linking, Alert } from "react-native";
...
const AppointmentModal = ({
  ...
  const ParticipantsFromFirm = () => {
    ...
    const clientParticipants = attendees.filter(attendee => extractDomain(attendee.email) !== domain);
    return <Fragment>
        ...
        if (extractDomain(attendee.email) === domain && !attendee.organizer) {
          return <View style={styles.attendee} key={attendee.email}>
        ...
        {clientParticipants.length > 0 && <>
            ...
            {clientParticipants.map(attendee => <View style={styles.attendee} key={attendee.email}>
                <Text style={styles.attendeeText}>{attendee.email}</Text>
              </View>)}
            <Button height={24} onPress={() => setModalVisible(false)} style={styles.hideButton}>
              Enrich Contacts
            </Button>
          </>}
      </Fragment>;
  ...
  return <Modal animationType="slide" transparent={true} visible={modalVisible}>
      <View style={styles.modalContainer}>
        ...
        <ScrollView>
          ...
          <Text style={styles.modalText}>
            Start time:{" "}
            {"start" in modalItem ? moment(new Date(start.dateTime)).format("YYYY-MM-DD HH:mm A") : ""}
          </Text>
          <Text style={styles.modalText}>
            End time:{" "}
            {"end" in modalItem ? moment(new Date(end.dateTime)).format("YYYY-MM-DD HH:mm A") : ""}
          </Text>
          <View style={styles.attendeeContainer}>{ParticipantsFromFirm()}</View>
          {attachments && <View style={styles.attendeeContainer}>
              <Text>Attachments:</Text>
              <TouchableOpacity onPress={() => handleOpenLink(attachments[0]?.fileUrl)}>
                <Text>{attachments[0]?.fileUrl}</Text>
              </TouchableOpacity>
            </View>}
        </ScrollView>
        <View style={styles.modalActionButton}>
          <Button height={40} onPress={() => setModalVisible(false)} style={styles.hideButton}>
            Cancel
          </Button>
        </View>
      </View>
