...
const Appointment = ({
  ...
  const getAllAppointment = async () => {
    setIsLoading(true); // access google authorization token with calendar scope

    const googleAuthorizationToken = await GoogleSignin.getTokens();
    await dispatch(getAppointmentByDate({
  ...
  const renderItem = ({
    ...
    return <TouchableOpacity onPress={() => toggleModal(item)}>
