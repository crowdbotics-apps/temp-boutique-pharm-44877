...
const Appointments = () => {
  ...
  const onDeletePress = async id => {
    setIsLoading(true);
    const tokens = await GoogleSignin.getTokens();
    await deleteAppointment(tokens.accessToken, id).then(() => {
  ...
  const renderItem = ({
    ...
    return <TouchableOpacity onPress={() => toggleModal(item)} style={styles.item} key={item.id}>
        <Fragment>
          ...
          <TouchableOpacity style={styles.deleteView} onPress={() => onDeletePress(item.id)}>
